...
```{r echo = FALSE, fig.align='center', comment='Figura 2. Evidencia de incendios en la region Cajamarca.'}
...
* Generar conocimiento y aprender la técnica de la Teledetección, además de aplicar los conocimientos aprendidos en clase.
...
```
<div style = "text-align: justify">Cabe mencionar que esta metodología podría extenderse para la presentanción final del trabajo, ya que dentro de nuestros objetivos especificos tenemos la idea de crear un modelo de predicción, el cual necesitará de más análisis de variables para que sea más consistente.

En este proyecto uitilizaremos dos lenguajes de programación, tanto R como Python. Para nuestro propósito se necesitará de varios paquetes, se mencionan a continuación:</div>
...
#### **5.1.1 Diagrama de barras de la acumulación de incendios a lo largo del tiempo**
...
### **5.2 Variabilidad estacional de la ocurrecia de incendios**
...
#### **5.3.1 Data histórica de incendios 2000-2018**
...
#### **5.3.2 Precipitación acumulada y Temperatura Máxima**
...
#### **5.3.3 Índice de Vegetación Normalizado (NDVI)**
...
La realizacion de el filtro de Savitzky Golay es reproducible y se encuentra disponible en el [repositorio de Github](https://github.com/ContrerasNk/AnalisOcurrenIncendiosR/tree/master/RasterData/Script/MODIS_SavGol.ipynb) del proyecto.
...
#### Indice de Vegatación Diferencial Normalizado 2000 -2018
...
start_date <- as.Date(data_csv$Date...1)

data_ndvi<-data.frame(start_date,data) %>% 
    mutate(mes=substr(start_date,6,7), ano=substr(start_date,1,4)) %>%                group_by(ano,mes) %>% summarise(ndvi = mean(data, na.rm = T))
...
ndvi2000 <- data.frame(tserie, col_ndvi) %>% 
  mutate(mes = substr(tserie,6,7)) %>% 
  dplyr::filter(substr(tserie,1,4) == 2000) %>% 
  group_by(mes) %>% summarise(ndvi2000 = mean(col_ndvi, na.rm=T))
...
X <- ggplot(df, aes(date, ndvimean)) +
     ...
     theme(plot.title = element_text(size = 16, hjust = 0.5, face = 'bold'),
           ...
           axis.title = element_text(size=12, face='bold'))+
     scale_x_date(date_labels = '%b', breaks = '1 month') +
     scale_y_continuous(breaks = seq(0.4,0.7,0.05), limits = c(0.4,0.7)) +
     ...
     geom_line(aes(date, ndvi2000), colour = rgb(233, 228, 91, maxColorValue = 255), size = 0.8) +
...
### 5.4 Análisis de variables: Análisis de comportamiento estacional de variables bioclimáticas por rangos de elevación
...
El objetivo de este apartado es elaborar un histograma sobre la distribución altitudinal de los incendios.

Cutervo
```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, out.width="825px"}
Cutervo <- st_read('Materiales/Cutervo.gpkg') %>% st_geometry() %>% sf_as_ee()
# Cajamarca <- st_read('Materiales/Cajamarca.gpkg') %>% st_geometry() %>% sf_as_ee()

Cutervo_dem <- ee$Image('CGIAR/SRTM90_V4')$clip(Cutervo)
# dem_caja_raster <- ee_as_raster(Cajamarca_dem, Cutervo)
# writeRaster(dem_caja_raster, 'RasterData/SRTM_90/CutervoSRTM.tif')
viz <- list(min= 100,
            max = 4500,
            palette = cpt(pal = 'gmt_GMT_dem1',n = 10))

Map$centerObject(Cutervo, zoom = 9)
...
dem   <- raster('RasterData/SRTM_90/CutervoSRTM.tif')
...
X <- ggplot(df, aes(df$elev)) + geom_histogram(binwidth = 100, fill='gray', col='black') +
  ...
  scale_y_continuous(breaks = seq(0,30,5), limits = c(0, 17), expand = c(0,0)) +
  scale_x_continuous(breaks = seq(0, 3500, 500), limits = c(500, 3700), expand = c(0,0))
...
Como podemos observar en el histograma los incendios se registran entre los 1000 y 4500 msnm, pero es entre los 3500 y 4500 principalmente donde la cantidad de incendios supera los 75 registros aproximadamente.


#### 5.4.2. Poligonizamos el DEM: Clases de altitud
...
# writeRaster(dem, 'RasterData/SRTM_90/CutervoAntPoly.tif')
# dem_poly <- rasterToPolygons(dem) %>% st_as_sf()
# dem_poly <- dem_poly %>% group_by(CutervoSRTM) %>% summarise(.)
```
 

#### 5.4.3. Variabilidad estacional de precipitación por elevación
...
X <- ggplot(df, aes(mes, elev))+ geom_raster(aes(fill=pp)) +
     ...
     ggtitle('Precipitación Mensual por Elevación', subtitle = 'De 1981 a 2018') + 
     labs(x = '', y = 'Altitud\n[msnm]', fill='[mm]') +
...
#### 5.4.4. Variabilidad estacional de temperatura máxima por elevación
