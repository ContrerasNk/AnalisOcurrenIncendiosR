...
```{r eval=TRUE, message=FALSE, echo = TRUE, warning=FALSE, cache=FALSE, fig.align='center'}
## Test de Shapiro-Wilk --------------------------------
shapiro.test(NDVI.Anomalias) # as.vector(pp.matrix)[as.vector(pp.matrix) <= 1]
...
área de estudio con un lag máximo de 3 meses, en donde las marcadas con ✇ corresponden a las correlaciones significativas $(p_{value} < 0.05)$ que se dan principalmente en los meses de Abril y Noviembre, se podría decir que de Mayo a Noviembre los valores poseen una correlación positiva principalmente en las correlaciones con lag 0 y 1.

# Discusion de resultados
* SERFOR debería realizar la gestión de quema controlada de residuos agrícolas.

* Se debería de contar con datos puntuales de la distribución de las quemas puede ser del INDECI. Con ello se pueden generar ROIs.
...
Por útimo el comportamiento estacional del NDVI sigue un patrón estacional similar al de la precipitación en cuanto al rango de elevación y estacionalidad, esta correlación fue verificada a través de una correlación rezagada que generó correlaciones positivas y significativas para las variables con retrasos de 0 y 1 meses. 
