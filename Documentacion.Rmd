...
leaflet() %>% 
  ...
  addPolygons(data = Anta, fillOpacity = 0, weight = 4, opacity = 1,
...
X <-  ggplot(df, aes(date, ppmean)) +
      ...
      theme(plot.title    = element_text(size = 16, hjust = 0.5, face = 'bold'),
            ...
            scale_x_date(date_labels = '%b', breaks = '1 month') +
            scale_y_continuous(breaks = seq(0,480,50), limits = c(0, 480)) +
...
X
```
En la anterior grafica podemos observar que
...
```{r, eval=TRUE, message=FALSE, eval=FALSE, echo = FALSE, warning=FALSE, cache=FALSE}

# La Region de interes es anta, pero puede variar según sigamos el proceso
Cajamarca <- st_read('Materiales/Cajamarca.gpkg')

# Generamos el centroide para la correcta visualizacion
centroid <- st_centroid(Cajamarca)
X <- (st_coordinates(centroid))[,"X"]
Y <- (st_coordinates(centroid))[,"Y"]
