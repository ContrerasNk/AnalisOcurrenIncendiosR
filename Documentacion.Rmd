...
## Correlación rezagada (Lagged correlation)
  ...
  Cabe destacar que la correlación no implica una causalidad o dependencia entre las variables y que debe ser utilizada una correlación paramétrica (Pearson) si las variables en conjunto cumplen con el supuesto de normalidad bivariada si no por el contrario se elige una correlación no paramétrica (Spearman). 
<br/>
...
```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, comment=FALSE, out.width="825px", fig.align='center', fig.cap='Figura 21. Estimación de la densidad para la ver la zonas más probables de incendios.'}
...
```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, out.width="825px", fig.align='center', fig.cap='Figura 22. Zona más problable de ocurrencia de incendios, ubicada en el sur, en la zona Andina oriental.'}
...
### Completación de datos con el método Savitzky Golay
  ...
  Este método en el servidor del google earth engine se encuentra en el ejemplo de [David Montero](https://github.com/davemlz/GEE_TimeSeries/blob/master/GEE_TimeSeries_SavGol.py) en **Python**, para este avance se tratará de aplicarlo en R.
...
<br/>
  ...
  En la anterior gráfica podemos observar la el NDVI, como indicador de la actividad fotosintética de la zona, máxima y mínima (de color rojo) y promedio histórico (en negro) entre los años 2000 - 2018, asímismo las líneas marrón, gris y verde representan la evolucion mensual de los años 2005, 2010 y 2016 respectivamente. Se observan caídas en la actividad fotosintética en los meses de mayor actividad fotosintética, entre junio a diciembre, ninguno ha superado el mínimo histórico sin embargo podemos concluir que la actividad fotosintética y por lo tanto la calidad de la vegetación se ve afectada en los meses de incendios en nuestra zona de estudio. 
...
# writeRaster(NDVI_Mensual_2000_2018, "RasterData/VarCorrelation/NDVI_Mensual_2000_2018.tif")
...
for (i in 1:length(date)) {
  print(i)
  for (j in 1:length(ndvi_lista)) {
    if (substr(ndvi_lista[[j]], 30, 33) == substr(date, 1, 4)[i] & substr(ndvi_lista[[j]], 35, 36) == substr(date, 6, 7)[i]) { # substr(ndvi_lista[[j]], 31, 34) for linux (un numero menos)
      x = 1
      lista1[[x]] = raster(ndvi_lista[j])
      x = x + 1
    }
...
writeRaster(Ndvi_mensual_stack, "RasterData/VarCorrelation/NDVI_SD_MENSUAL.tif")
```
...
# writeRaster(pp_Mensual_2000_2018, "RasterData/VarCorrelation/PP_Mensual_2000_2018.tif")
...
# pppp <- stack("RasterData/PISCO/PISCOp_v11_area.tif")
# Ndvi_mensual_stack <- raster::resample(Ndvi_mensual_stack, pppp)
PP_mensual_stack <- crop(PP_mensual_stack, area, snap = "out")
writeRaster(PP_mensual_stack, "RasterData/VarCorrelation/PP_MEAN_MENSUAL.tif")
```
...
# Ndvi_mensual_stack <- raster::resample(Ndvi_mensual_stack, pppp)
PP_mensual_stack <- crop(PP_mensual_stack, area, snap = "out")
writeRaster(PP_mensual_stack, "RasterData/VarCorrelation/PP_SD_MENSUAL.tif")
```
...
# ## Promedios mask
# NDVI_MEAN_MK <- raster("RasterData/VarCorrelation/NDVI_MENSUAL_MASK.tif")
# PP_MEAN_MK <- raster("RasterData/VarCorrelation/PP_MENSUAL_MASK.tif")

# ## Sds mask
# NDVI_SD_MK <- raster("RasterData/VarCorrelation/NDVI_SD_MENSUAL_MASK.tif")
# PP_SD_MK <- raster("RasterData/VarCorrelation/PP_SD_MENSUAL_MASK.tif")
...
for (i in 1:12) {
  ...
  while (n <= nlayers(PP_2000_2018)) {
    lista5[[p]] = PP_2000_2018[[n]]
    n = n + 12
    p = p + 1
  }
  Pp_mensual[[i]] = ((stack(lista4) - PP_MEAN[[i]]) / PP_SD[[i]])
  print(i)
}
```
...
### NDVI
data_ndvi <- ndvi_velox$extract(sp = region, fun = function(x){ mean(x, na.rm = T) }) %>%
  as.vector()
