...
En este análisis se evaluará la estacionalidad de las variables bioclimáticas que favorecen la ocurrencia de incendios, desde el año 2000 al 2018. 
...
### Objetivo general
  ...
  El objetivo principal de este estudio es analizar la estacionalidad de variables bioclimáticas para la posterior aplicación de un modelo predictivo para las zonas con condiciones favorables para la ocurrencia de incendios. 

### Objetivos específicos

* Conocer el comportamiento de los incendios a escala mensual y anual para el período 2000 - 2018.

* Definir nuestra área de estudio como una de las regiones con mayor probabilidad de incendios.

* Realizar un análisis temporal con las variables bioclimáticas de precipitación acumulada, temperatura máxima y calidad de la vegetación.     
* Analizar el comportamiento estacional de las variables bioclimáticas por rangos de elevación.

* Verificar si existe alguna relación entre las variables bioclimáticas en el área de estudio.

* Generar una metodología reproducible de investigación el cual sirva a posteriori para generar un modelo predictivo.

* Aplicar los conocimientos, nociones y conceptos adquiridos en el curso de Teledetección.
...
## Fuego
  ...
  Según Rodríguez (2010), el fuego se define como la liberación de energía en forma de luz y calor producida por la combustión de un cuerpo, lo que lleva a una descomposición rápida del combustible originada por una temperatura elevada y la presencia de oxígeno.  
...
## Triángulo del fuego (origen)
  ...
  Para que el fuego se pueda presentar se necesita de tres elementos: Combustible, calor y oxígeno. Si no llegase a presentarse uno de estos, la conflagración no se produciría. La (Figura) en el recuadro (a.) muestra esta representación. 
...
* **La Gran Triada.** Otra representación característica es la gran tríada en la cual se relacionan los combustibles, el tiempo atmosférico y la topografía, donde el primero de estos es el único elemento donde el hombre puede actuar directamente, a diferencia de los dos últimos, los cuales no pueden modificarse. La (Figura 5) en el recuadro (b.) muestra esta representación.  

<br/>

```{r echo = FALSE, fig.align='center', out.width="70%", fig.cap='Figura 5. a. Representación del Triángulo de Fuego. & b. La gran Triada. Fuente: Comisión Nacional Forestal (2010). Incendios Forestales, guía práctica para comunicadores. (Tercera edición), 6-8.'}
...
El estudio de IFs sigue siendo un desafío para la ciencia y el manejo forestal, debido a la interacción entre factores naturales y antrópicos (Lippok et a., 2013).  
...
## Regiones con mayor cantidad de incendios
  ...
  En los últimos años los IFs han impactado severamente varias regiones del Perú, como Cusco, Apurímac, Lambayeque, Piura y Cajamarca (INDECI, 2013; El Comercio, 2018).  
...
```{r echo = FALSE, fig.align='center', comment='', out.width="70%", fig.align='center', fig.cap='Figura 8. Cantidad de incendios registrados por regiones en el Perú desde el 2000 al 2018.'}
...
La ecuación para la estimación de la densidad para un punto $x$ sería:  
...
## Filtro Digital Savitzky-Golay
 ...
 Tiene como objetivo la disminucion del ruido de una curva. Esta curva la definimos como los valores de cada pixel a lo largo de un periodo dado, en nuestro caso el valor del píxel será el NDVI. 
...
* $2n+1$: Es el ancho del filtro, es decir los puntos necesarios para calcular la curva polinomial ajustada de grado $p$, generalmente n es igual a 5 por lo tanto el ancho del filtro son 11 puntos, donde el punto central $k$ de la curva polinomial.

* ${({Y}_{k+i})}$: Es la funcion polinomial ajustada de grado $p$ empleada para la posicion $k+i$.  
...
*Este método nos servirá para completar los datos del índice NDVI, para lo años de estudio.*  


```{r echo = FALSE, fig.align='center', comment='', out.width="70%", fig.align='center', fig.cap='Figura 11.Animación que muestra la aplicación de suavizado, pasando los datos de izquierda a derecha. La línea roja representa el polinomio local que se utiliza para ajustar un subconjunto de datos. Los valores suavizados se muestran como círculos.'}
knitr::include_graphics('Img/savgol.gif')
```

## Correlación rezagada (Lagged correlation)
  La correlación rezagada calcula el grado de asociación entre dos o más variables a través de lags o retrasos temporales definidos a escala diaria, quincenal o mensual. Estos lags son por lo tanto la cantidad de tiempo necesario en donde las variables poseen un mayor grado de correlación. 
  Este concepto tiene aplicaciones útiles para caracterizar las variables bioclimáticas ya que los efectos entre estas no se relacionan de manera directa e inmediata. 
  Cabe destacar que la correlación no implica una causalidad o dependencia entre las variables. 
  
```{r echo = FALSE, fig.align='center', comment='', out.width="70%", fig.align='center', fig.cap='Figura 12. Se muestra un ejemplo de relación rezagada con un lag máximo de tres meses entre las variables de anomalía mensual del NDVI y de precipitación mensual para determinadas coberturas vegetales'}
knitr::include_graphics('Img/LaggedCorrelation.jpg')
```  
<br/>
...
## Localización
  ...
  El área de estudio será sustenta con los procedimientos de nuestra metodología, para ello, se aplicó la función kernel de densidad para estimar los lugares en donde se darían más los incendios. Así, tenemos como área de estudio un área que abarca la región de Cusco y Apurimac. La probabilidad que se encuentra en el área es mayor a 5%.  

<br/>

```{r echo = FALSE, out.width="100%", fig.align='center', fig.cap='Figura 13. Zona de mayor cantidad de incendios ubicada en los Andes orientales.'}
...
Para nuestra área de estudio, se hará uso del NDVI procesado a resolucion de 250 m, el cual será filtrado según su banda de calidad (QA). 

<br/>

```{r echo = FALSE, out.width="70%", fig.align='center', fig.cap='Figura 15. MOD13Q1.006 Terra Vegetation Indices 16-Day Global 250m.'}
knitr::include_graphics('Img/modis_ndvi.gif')
```

## Datos PISCO

El producto grillado [PISCO](https://iridl.ldeo.columbia.edu/SOURCES/.SENAMHI/.HSR/.PISCO/?Set-Language=es#info) (Peruvian Interpolated data of the SENAMHI’s Climatological and hydrological Observations) es una base de datos espacial  de precipitación acumulada y temperatura (maxima y minima), a una resolución de grilla de 0.05º(5 km) para una serie que se inicia en enero de 1981. Sus productos poseen una escala diaria y mensual y se encuentran disponibles hasta Febrero del año 2018.  

```{r echo = FALSE, out.width="70%", fig.align='center', fig.cap='Figura 16. Investigadores del senamhi publican artículo científico sobre producto grillado de precipitación..'}
...
```{r eval=TRUE, echo=FALSE, warning=FALSE, cache=FALSE, out.width="100%", fig.align='center', fig.cap='Figura 17. Mapa de elevaciones del área de estudio para las CFOI. Fuente: Google Earth Engine - SRTM V4.'}
...
Cabe mencionar que todos los procesos "pesados" que normalmente consumirían muchos recursos de una computadora convencional, lo realizamos en la plataforma ***Google Earth Engine (GEE)***. Para conectar con esta plataforma se utilizó las APIS para los lenguages seleccionados. Con R usamos el paquete RGEE y para Python geemap, los  procedimientos reproducibles en el lenguaje Python, para la descarga de datos de NDVI, se encuentran en el siguiente [enlace](https://github.com/ContrerasNk/AnalisOcurrenIncendiosR/blob/master/RasterData/Resultados/Script/NDVI_MODIS.ipynb).
  ...
  ***Como ejemplo, para iniciar sesión en la plataforma GEE por medio de RGEE lo hacemos con el siguiente script, conectando además con nuestra cuenta de Google Drive.*** 
...
```{r echo = FALSE, fig.align='center', fig.cap = 'Figura 18. Metodología de la investigación.', out.width="70%"}
knitr::include_graphics('Img/METODOLOGIA-FINAL.jpg')
```
 Cabe mencionar que esta metodología podría extenderse, con el seguimiento para investigaciones posteriores, ya que esta investigación solo es un EDA, el comienzo para generar un modelo de predicción. 


## Diagrama de barras de la acumulación de incendios a lo largo del tiempo

  Se analilzó la tendencia de la ocurrencia de los incendios a lo largo del tiempo, para ello, se necesito de un histograma. En este apartado generaremos un histograma de cantidad de incendios en el tiempo. Tendremos como resultado en qué años predominan estos fenómenos. 

<br/>

```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, out.width="80%", fig.align='center', fig.cap='Figura 19. Eventos de incendios a lo largo de los años 2000-2018.'}
# Incendios acumulados por mes
...
ggplot(ts, aes(x=Date, y=nfires)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5, face = 'bold', size=15, color = "#020722"),
        ...
        axis.title.x = element_text(size = 0.1, hjust = 0.5, face = 'bold', vjust = 0.5, color = "#020722")) + 
...
```{r echo = FALSE, fig.align='center', fig.cap = 'Figura 20. SPI para los años seleccionado para nuestra área de estudio Puno-Cusco.', out.width="70%"}
knitr::include_graphics('Img/spi_desertificacion.png')
```

## Distribución de los incendios por años de 2000 - 2018

  En el apartado anterior generamos un histograma con el fin de ver el comportamiento acumulativo de los incendios desde 2000 al 2018, para que se entienda mejor la distribución de los puntos de incendios se procedió a plotear los puntos de una manera más interactiva, así ver también, cómo se comporta la distribución de incendios a traves de los años de estudio.  
...
```{r leaflet, eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, out.width="825px", fig.align='center', fig.cap = 'Figura 21. Mapa interactivo para ver la distribución de incendios por años.'}
# Incendios acumulados por mes
...
### Estimación de densidad de probabilidad 2D: función de densidad no paramétrica
  ...
  Se suavizó la serie de incendios (puntos), en función a una matriz, en este caso 250 x 250, tambien definido el tamaño del pixel 0.8 s, además se tuvo en cuenta la matriz movil o kernel (3x3, 5x5, etc).
...
Basicamente, esta función del paquete MASS, en R, varía por ubicación (asignandole peso), sus argumentos de coordenadas son los límites de la matriz ya definida.  

<br/>

```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, comment=FALSE, out.width="825px", fig.align='center', fig.cap='Figura 22. Estimación de la densidad para la ver la zonas más probables de incendios.'}
...
```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, out.width="825px", fig.align='center', fig.cap='Figura 23. Zona más problable de ocurrencia de incendios, ubicada en el sur, en la zona Andina oriental.'}
...
Una vez filtrado el raster de NDVI por su banda QA, por coste computacional es necesario hacer un resampleo considerando una resolución igual a la proporcionada por el producto grillado PISCO.
...
Se puede inferir que en los meses de mayor incendios (de julio a enero) se presentaron valores  por debajo de la media, alcanzando incluso  a los mínimos históricos de precipitación acumulada mensual.

Se ve que el mes de octubre es la excepción (2010 y 2016), donde la precipitación supera a la media histórica. Aunque, en el gráfico de número de incendios por meses, se ve justo en ese mes hay gran cantidad de incendios, bordeando los 150, para luego crecer con gran pendiente en el mes de noviembre, llegando mas de 400. Esto hace pensar que una pequeña variación en la precipitación, aprox de 30mm, afecta de una manera considerable a la cantidad de incendios.

El año "normal" donde no hay gran cantidad de incendios históricos, el año 2000, tiene un comportamiento donde la mayoría de los meses supera la media histórica.
...
Se puede inferir que en los meses de mayores incendios (de julio a enero) se presentaron valores  por encima de la media, alcanzando incluso a los máximos históricos de temperatura máxima. Aunque vemos que el año 2005 no sigue esa tendencia, solo para los meses setiembre y octubre. 

También se puede ver que la estacionalidad no es tan marcada, como la precipitación, incluso la temperatura tiene una tendencia creciente en los meses con menor cantidad de incendios.
...
Se observan caídas en la actividad fotosintética en una estacionalidad marcada de mayo a noviembre, los meses de mayor ocurrencia de incendios (julio a enero), en donde los valores para el año 2005 están muy próximos a superar el mnimo histórico, por lo tanto la calidad de la vegetación se ve afectada en los meses de incendios en la zona de estudio.

Igual que la precipitación tiene una tendencia marcada, esto haría pensar que ambas variables tiene una relación más próxima, sin incluir la precipitación.

Se realizará más observaciones, pero ahora las varibles bioclimáticas estacionales con las altitudes.


## Análisis de comportamiento estacional de variables bioclimáticas por rangos de elevación

Debido a que la distribución espacial de IFs, en el Perú, concentrada en regiones de alta montaña (entre 1500 y 4000 msnm) y en la región andina oriental (donde se encuentra el área de estudio elegido), así se consideró incluir los rangos de altitud como variable en nuestro CFOI.

El objetivo de este apartado es poder graficar la variabilidad mensual de la lluvia, temperatura máxima y el NDVI por rangos altitudinales y así establecer un umbral de elevación, a partir del cual dicho contrastre estacional deja de ser notorio.

### Variabilidad estacional de precipitación por elevación

```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, fig.align='center', fig.cap='Figura 31. Variabilidad estacional de precipitación por elevación.'}
pp_velox <- velox('RasterData/PISCO/PISCOp_v11_area.tif')
...
Al igual que el gráfico estacional anterior, de esta variable, tiene una marcada estacionalidad y a mayor altitud tiende a disminuir, pero no es tan marcado.

### Variabilidad estacional de temperatura máxima por elevación

```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, fig.align='center', fig.cap='Figura 32. Variabilidad estacional de temperatura máxima por elevación.'}
...
En este gráfico, vemos que la variable es más progresiva respecto a la altitud, también, hay un aumento repentino en el mes de noviembre, justo el mes de mayor cantidad de incendios.

### Variabilidad estacional del NDVI por elevacion

```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, fig.align='center', fig.cap='Figura 33. Variabilidad estacional del NDVI por elevacion.'}
...
Esta variable tiene una marcada estacionalidad, además de un marcado cambio al aumento de altitud (disminuye). 
...
Luego de analizar graficamente la estacionalidad de las variables bioclimáticas y su compartamiento en función de los rangos de elevación se identificó un comportamiento similar en las variables de precipitación acumulada y NDVI mensual para ambos casos, debido a ello se propuso verificar si existe algun grado de asociación entre ellas en el área de estudio.

Para lograr aquello, se dispuso en primer lugar calcular las anomalías para ambas variables, según Wilks, D. S., (1995) debido a la variación estacional de los eventos climáticos el uso de datos de anomalía estandarizada en lugar de valores medios es más conveniente para investigar la relación del NDVI mensual con respecto a la precpiticación mensual.

```{r eval=FALSE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, fig.align='center'}
# NDVI Promedios históricos -------------------------------
...
for (i in 1:12) {
  ...
  PP_mensual[[i]] = calc(stack(lista6), fun = sd)
  print(i)
}
...
# writeRaster(NDVI_MENSUAL, "RasterData/VarCorrelation/NDVI_SD_MENSUAL.tif")
# writeRaster(PP_MENSUAL, "RasterData/VarCorrelation/PP_SD_MENSUAL.tif")
# writeRaster(NDVI_MENSUAL_MASK, "RasterData/VarCorrelation/NDVI_SD_MENSUAL_MASK.tif")
# writeRaster(PP_MENSUAL_MASK, "RasterData/VarCorrelation/PP_SD_MENSUAL_MASK.tif")
...
df_ndvi <- read.csv("RasterData/VarCorrelation/NDVI_ANOM.csv")
ndvi.matrix <- as.matrix(df_ndvi[1:18,3:14])

## Matriz PP de 2000 - 2017
df_pp <- read.csv("RasterData/VarCorrelation/PP_ANOM.csv")
pp.matrix <- as.matrix(df_pp[,3:14])
```

Una vez obtenida las anomalías mensuales promediadas del NDVI y precipitación acumulada para el área de estudio se procede a evaluar el tipo de distribución, el cual nos determinará si la correlación utilizada será paramétrica o no paramétrica.
Se analiza en primer momento la gráfica de estas variables a través de un histograma, donde se aprecia visualmente la distribución siendo para el caso del NDVI una distribución normal en donde la mayor frecuencia de valores dentro del rango de la media igual a 0. 

```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, fig.align='center', fig.cap="Figura 34. Histograma de las anomalías de NDVI mensual promediado sobre el área para el período 2000 - 2017"}

# arriba pp.matrix esta definidaaaaaa

data_frame(val = as.vector(ndvi.matrix)) %>% 
ggplot(aes(val)) + 
  ...
  theme(plot.title    = element_text(size = 20, hjust = 0.5, face = 'bold'),
        ...
        axis.title.y = element_text(size = 12, hjust = 0.5, face = 'bold', vjust = 0.5, color = "#020722"),
        axis.title.x = element_text(size = 12, hjust = 0.5, face = 'bold', vjust = 0.5, color = "#020722")) +
  scale_y_continuous(breaks = seq(0,25,5), limits = c(0, 40), expand = c(0,0)) +
  scale_x_continuous(breaks = seq(-1.7, 1.7, 0.3), limits = c(-1.7, 1.7), expand = c(0,0))
```

```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, fig.align='center', fig.cap= "Figura 35. Histograma de las anomalías de precipitación acumulada mensual promediado sobre el área para el período 2000 - 2017"}

data_frame(val = as.vector(pp.matrix)) %>% 
ggplot(aes(val)) + 
  geom_histogram(binwidth = 0.1, fill='gray', col='black') +
  theme_bw() + labs(x = '', y = '') +
  ylab(label = 'Frecuencia de anomalías\n') +
  ...
  theme(plot.title    = element_text(size = 20, hjust = 0.5, face = 'bold'),
        ...
        axis.title.y = element_text(size = 12, hjust = 0.5, face = 'bold', vjust = 0.5, color = "#020722"),
        axis.title.x = element_text(size = 12, hjust = 0.5, face = 'bold', vjust = 0.5, color = "#020722")) +
  scale_y_continuous(breaks = seq(0,40,5), limits = c(0, 30), expand = c(0,0)) +
  scale_x_continuous(breaks = seq(-1, 3, 0.2), limits = c(-1, 2.5),expand = c(0,0))
```

Si y solo si nuestras dos variables son variables aleatorias independientes e identicamente distribuidas (vaiid's) bajo una distribución normal, se procederá a realizar la prueba de normalidad bivariada para verificar si en efecto es posible realizar una correlación paramétrica entre estas.

Por el contrario, si nuestras variables de manera independiente no cumplen la distribución normal se descarta esta posibilidad y se asume una correlación no parámetrica, que para nuestro caso fue la correlación de Spearman.

Para ello, estimaremos si una variable tiene una distribución normal o no planteando la hipótesis nula $(H_0)$ de que una muestra proviene de una distribución normal, es decir:

$$H_0: X \sim \mathcal{N}(\mu,\sigma^2)$$

Y una hipótesis alternativa, donde la distriución no es normal:

$$H_1: X \nsim \mathcal{N}(\mu,\sigma^2)$$

Antes de evaluar los test de normalidad, el $p_{value}$ se debe de conocer definir nivel de significanza $\alpha$  como la probabilidad de rechazar la hipótesis nula $(H_0)$ cuando ésta es verdadera, conocido también como Error tipo I. Normalmente esta probabilidad posee el valor de $0.05$.

El $p_{value}$, es una cantidad que nos indica qué tan insólitos, extraños, raros o inusuales son los resultados en la muestra del estudio, considerando que la hipótesis nula (H0) establecida a priori sea verdadera. Si esta posee valores más altos que el nivel de significancia $\alpha$ establecido a priori significa que la $(H_0)$ no debe ser rechazada. 

Los test de normalidad utilizados fueron Shapiro-Wilk y Anderson Darling.

```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, fig.align='center', comment= '', results='hold'}
## Test de Shapiro-Wilk --------------------------------
NDVI.Anomalias <- as.vector(pp.matrix)[as.vector(pp.matrix) <= 1] 
PRECIPITACION.Anomalias <- as.vector(pp.matrix)[(as.vector(pp.matrix) >= -1) & (as.vector(pp.matrix) <= 1)]
```

```{r eval=TRUE, message=FALSE, echo = TRUE, warning=FALSE, cache=FALSE, fig.align='center', comment= '', results='hold'}
## Test de Shapiro-Wilk --------------------------------
shapiro.test(NDVI.Anomalias) 
shapiro.test(PRECIPITACION.Anomalias)

## Test de Anderson Darling ----------------------------
ad.test(NDVI.Anomalias)
ad.test(PRECIPITACION.Anomalias)
```

Vemos que en ambos casos el valor de probabilidad $p_{value}$ para las anomalías del NDVI es  superior a nuestro nivel de significación $\alpha = 0.05$ por lo que no rechazamos la hipótesis nula. Mientras que para el caso de las anomalías de precipitación es inferior a nuestro nivel de significación $\alpha = 0.05$ por lo que rechazamos la hipótesis nula. Por lo tanto la anomalía de NDVI y precipitación acumulada no sigue en conjunto una normalidad bivariada.

Por ello, la correlación no paramétrica de Spearman fue aplicada a las variables con un lag máximo de 3 meses tal y como se muestra en la siguiente gráfica.
...
```{r eval=TRUE, message=FALSE, echo = FALSE, warning=FALSE, cache=FALSE, fig.align='center', fig.cap= "Figura 36. Correlación rezagada para las anomalías de NDVI y precipitación acumulada mensual promediados sobre el área para el período 2000 - 2017"}
...
ggplot(melted_a, aes(pp,ndvi))+ geom_raster(aes(fill=corr)) +
  ...
  labs(x = 'Anomalía de precipitación acumulada mensual\n[mm/mes]', y = 'Anomalía de NDVI mensual', fill='') +
  theme_bw() +
  theme(plot.title    = element_text(size = 15, hjust = 0.5, face = 'bold'),
        plot.subtitle = element_text(size = 11, hjust = 0.5, face = 'italic'),
        axis.text.x   = element_text(size = 10, hjust = 1),
        axis.text.y   = element_text(size = 10),
        axis.title.y  = element_text(size = 11, hjust = 0.5, face = 'bold', vjust = 0.5),
        axis.title.x  = element_text(size = 11, hjust = 0.5, face = 'bold', vjust = 0.5)) +
  geom_text(aes(2, 2, label = "X"), color = "black", size = 6) + 
  geom_text(aes(3, 3, label = "X"), color = "black", size = 6) + 
  geom_text(aes(3, 5, label = "X"), color = "black", size = 6) +
  geom_text(aes(5, 5, label = "X"), color = "black", size = 6) +
  geom_text(aes(7, 7, label = "X"), color = "black", size = 6) +
  geom_text(aes(7, 8, label = "X"), color = "black", size = 6) +
  geom_text(aes(7, 9, label = "X"), color = "black", size = 6) +
  geom_text(aes(7, 10, label = "X"), color = "black", size = 6) + 
  geom_text(aes(9, 9, label = "X"), color = "black", size = 6) + 
  geom_text(aes(9, 9, label = "X"), color = "black", size = 6) +
  geom_text(aes(11,1, label = "X"), color = "black", size = 6) +
  geom_text(aes(11, 11, label = "X"), color = "black", size = 6) +
  geom_text(aes(11, 12, label = "X"), color = "black", size = 6) 

```

Se aprecia la correlación de las anomalías de precipitación acumulada mensual con respecto a las anomalías mensuales de NDVI promediadas sobrenuestra 
área de estudio con un lag máximo de 3 meses, en donde las marcadas con "x" corresponden a las correlaciones significativas $(p_{value} < 0.05)$ que se dan principalmente en los meses de Abril y Noviembre los valores poseen una correlación positiva principalmente en las correlaciones con lag 0 y 1.

# Discusión y Conclusiones

La data es dudosa para el año 2019 (sobre registro de incendios) es atípico ya que los incendios se triplican en algunos meses pero hasta el momento no ha habido algún estudio en el que se diga que el año 2019 ha sido considerado como año de sequía extrema (como en el caso del año 2005, 2010 y 2016).

De lo expuesto anteriormente, se concluye que es importante caracterizar nuestras variables bioclimáticas por rangos de elevación, en el caso de la precipitación, no hay gran cambio en base a la altitud, pero si sigue un patron estacional, coincidiendo con el gráfico estacional incendios, aquel constraste estacional desaparece registrandose precipitaciones acumuladas mensuales por debajo de 200 mm y conviertiendose en condición favorable para la ocurrencia de incendios, en el caso de la temperatura máxima, por su parte, se observa una homogeneidad de sus valores hasta los 2500 msnm, ya por este umbral se denota un ligero contraste entre la epoca seca y la época húmeda.

Los meses de abril a noviembre respecto a los demás, siendo estos considerados como el periodo con menor actividad fotosintética y condición favorable para la ocurrencia de incendios, con valores debajo de los 0.40 de NDVI.

El comportamiento estacional del NDVI sigue un patrón similar al de la precipitación en cuanto al rango de elevación y estacionalidad, esta correlación fue verificada a través de una correlación rezagada que generó correlaciones positivas y significativas para las variables con retrasos de 0 y 1 mes para el período de Abril a Noviembre. 

Entonces podemos llegar a la conclusión que si analizamos el comportamiento estacional de distintas variables, podemos construir un modelos de pronóstico de incendios, el cual puede llegar a ser un optima herramienta de gestion y conservación de ecosistemas vulnerables frente al cambio climático y la acción antrópica.
...
[Wilks, D. S. (2011). Statistical methods in the atmospheric sciences (Vol. 100). Academic press.](http://nwafiles.nwas.org/digest/papers/2007/Vol31No2/Pg167-Glahn-BookReview.pdf)
